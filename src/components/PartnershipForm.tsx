  ...
  return (
    <section id="contact" className="bg-alabaster py-24 relative overflow-hidden">
      <div className="max-w-3xl mx-auto px-6 relative z-10">
        ...
        {/* Wizard Container */}
        <div className="bg-white border border-brand-border rounded-2xl p-8 shadow-xl min-h-[400px] flex flex-col justify-between">
          ...
          <div className="flex-grow flex items-center justify-center">
            ...
            {isSuccess ? (
              <div className="text-center space-y-4 py-8">
                ...
                <h3 className="text-2xl font-heading-sans font-bold text-text-primary">Pengajuan Kemitraan Terkirim!</h3>
              ...
              <div className="w-full">
                ...
                {step === 1 && (
                  <div className="space-y-6 animate-fade-in">
                    <h3 className="text-xl font-heading-sans font-bold text-text-primary text-center">Siapakah representasi Anda?</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {roles.map((r) => (
                        <button
                          ...
                          className={cn(
                            "p-5 rounded-xl border text-left font-medium text-sm transition-all duration-200 bg-offwhite",
                            formData.role === r.id
                              ? "border-teal-accent text-teal-accent shadow-[0_0_15px_rgba(13,148,136,0.1)]"
                              : "border-brand-border hover:border-teal-accent/50 text-text-primary"
                          )}
                        >
                          {r.label}
                        </button>
                ...
                {step === 2 && (
                  <div className="space-y-6">
                    <h3 className="text-xl font-heading-sans font-bold text-text-primary text-center">Layanan apa yang Anda perlukan?</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {needs.map((n) => (
                        <button
                          key={n.id}
                          ...
                          className={cn(
                            "p-5 rounded-xl border text-left font-medium text-sm transition-all duration-200 bg-offwhite flex justify-between items-center",
                ...
                {step === 3 && (
                  <div className="space-y-6">
                    <h3 className="text-xl font-heading-sans font-bold text-text-primary text-center">Ceritakan singkat tentang proyek Anda</h3>
                    <div className="max-w-lg mx-auto">
                      <textarea
                        ...
                        className="w-full bg-offwhite border border-brand-border rounded-xl p-4 font-sans text-sm text-text-primary placeholder-text-muted/65 focus:ring-2 focus:ring-teal-accent focus:bg-white outline-none transition-all"
                      />
                    </div>
                  </div>
                )}
                ...
                {step === 4 && (
                  <div className="space-y-6">
                    <h3 className="text-xl font-heading-sans font-bold text-text-primary text-center">Bagaimana cara terbaik menghubungi Anda?</h3>
                    <div className="max-w-md mx-auto space-y-4">
                      <div>
                        ...
                        <input
                          ...
                          required
                          className="w-full bg-offwhite border border-brand-border rounded-xl px-4 py-3 font-sans text-sm text-text-primary placeholder-text-muted/65 focus:ring-2 focus:ring-teal-accent focus:bg-white outline-none transition-all"
                        />
                      </div>
                      <div>
                      ...
                      {errorMsg && <p className="text-xs text-red-500 mt-2 font-mono">{errorMsg}</p>}
                    </div>
                  </div>
                )}
          ...
          {!isSuccess && (
            <div className="flex justify-between items-center mt-8 pt-4 border-t border-brand-border">
              <button
                ...
                disabled={step === 1}
                className={cn(
                  "flex items-center gap-2 text-xs font-mono tracking-wider text-text-muted uppercase hover:text-text-primary disabled:opacity-30 disabled:pointer-events-none transition-colors",
                  step === 1 && "invisible"
                )}
              >
              ...
              {step < 4 ? (
                <button
                  ...
                  className="flex items-center gap-2 bg-offwhite border border-brand-border text-text-primary text-xs font-mono tracking-wider uppercase px-5 py-2.5 rounded-xl hover:border-teal-accent transition-colors disabled:opacity-50"
                >
                ...
                <button
                  ...
                  className="flex items-center gap-2 bg-teal-accent hover:bg-brand-slate text-text-inverse font-heading-sans text-xs font-bold tracking-wider uppercase px-6 py-3 rounded-xl transition-all"
                >
